...
function roleFrom(r: string | undefined): AuthUser['ruolo'] { return r === 'OWNER' || r === 'STAFF' ? r : 'STAFF'; }
...
export function createSupabaseAuth(client: Client): AuthService {
  return {
    ...
    // STAFF: invia OTP SOLO se l'email è autorizzata e attiva (nessuna password).
...
// Backup/ripristino cloud dello stato completo (app_state). Build-safe: client iniettato.
// Permette il recupero automatico dopo cambio dispositivo (login → pullState → ripristino).
export function createSupabaseStateSync(client: Client, orgId: string) {
  return {
    // Salva l'intero stato serializzato (personale, reparti, matrici, ferie, desiderata, richieste, profilo, impostazioni).
    async pushState(data: unknown): Promise<{ ok: boolean }> {
      const { error } = await client.from('app_state').upsert({ org_id: orgId, data, updated_at: new Date().toISOString() }, { onConflict: 'org_id' });
      return { ok: !error };
    },
    // Recupera lo stato dal cloud al login su un nuovo dispositivo.
    async pullState(): Promise<unknown | null> {
      const { data } = await client.from('app_state').select('data').eq('org_id', orgId).maybeSingle();
