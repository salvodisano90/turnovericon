...
export { passwordIssue }; // fonte unica (policy completa: 8+, Aa1)
...
export const LocalAuthProvider: AuthProvider = {
  ...
  async setNewPassword(email, code, password) {
    ...
    try { await AsyncStorage.removeItem(K_SESSION); } catch { /* no-op */ } // mai auto-login dopo reset: l'utente torna al login
    return { ok: true };
  },
