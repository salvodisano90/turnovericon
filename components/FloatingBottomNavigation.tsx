    ...
    <View pointerEvents="box-none" style={[styles.wrap, { paddingBottom: (insets.bottom || 0) + 24 }]}>
      <View style={[styles.bar, { backgroundColor: BAR_BG, borderColor: GLASS.border, overflow: 'hidden' }]}>
        ...
        {routes.map((route) => {
          ...
          return (
            <AnimatedTab key={route.key} focused={focused} onPress={onPress} icon={m.icon} colors={colors} badge={m.badgeKey === 'pending' ? pendingCount : 0} onTabLayout={(x: number, w: number) => setTabL((prev) => (prev[route.key] && prev[route.key].x === x && prev[route.key].w === w ? prev : { ...prev, [route.key]: { x, w } }))} />
          );
        })}
      </View>
    </View>
...
function AnimatedTab({ focused, onPress, icon, colors, badge = 0, onTabLayout }: { focused: boolean; onPress: () => void; icon: string; colors: any; badge?: number; onTabLayout: (x: number, w: number) => void }) {
