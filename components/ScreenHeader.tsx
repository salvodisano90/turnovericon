    ...
    <View style={[styles.bar, { backgroundColor: colors.card, borderBottomColor: colors.separator }]}>
      <View style={styles.row}>
        <View style={styles.titleRow}>
          <Pressable onPress={() => (router.canGoBack() ? router.back() : router.replace('/'))} hitSlop={8} style={[styles.back, { backgroundColor: colors.card2 }]} accessibilityLabel="Indietro">
...
const styles = StyleSheet.create({
  back: { width: 44, height: 44, borderRadius: 22, alignItems: 'center', justifyContent: 'center', marginRight: 4 },
  bar: { borderBottomWidth: StyleSheet.hairlineWidth, paddingBottom: 10 },
  row: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', minHeight: 72, paddingHorizontal: 14 },
  ...
  mswLabel: { fontSize: 13, fontWeight: '700', minWidth: 120, textAlign: 'center' },
});
