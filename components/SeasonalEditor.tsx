// components/SeasonalEditor.tsx — editor stagioni inline (date + matrice per stagione) + banner validazione.
// Riusa validateSeasonalConfig (layer esistente). Nessuna nuova struttura dati: opera su SeasonalConfig.
...
export default function SeasonalEditor({ cfg, onChange, matrici }: {
  cfg: SeasonalConfig; onChange: (c: SeasonalConfig) => void; matrici: { id: string; label: string }[];
}) {
  const { colors } = useTheme();
  const ids = useMemo(() => matrici.map((m) => m.id), [matrici]);
  const v = useMemo(() => validateSeasonalConfig(cfg, ids), [cfg, ids]);

  const setRange = (s: Season, patch: Partial<SeasonRange>) => onChange({ ...cfg, [s]: { ...cfg[s], ...patch } });
  ...
  return (
    <View>
      ...
      {SEASONS.map((s) => {
        const r = cfg[s.key] || ({} as SeasonRange);
        return (
          <View key={s.key} style={[styles.card, { backgroundColor: colors.card, borderColor: colors.line }]}>
            ...
            <Text style={[styles.dl, { color: colors.text3, marginTop: 8 }]}>Matrice</Text>
            <View style={styles.chips}>
              {matrici.map((m) => (
                <PressableScale key={m.id} onPress={() => setRange(s.key, { matrice: m.id })}>
                  <View style={[styles.chip, { backgroundColor: r.matrice === m.id ? colors.blueSoft : colors.card2, borderColor: r.matrice === m.id ? colors.blue : colors.line }]}>
                    <Text style={[styles.chipTxt, { color: r.matrice === m.id ? colors.blue : colors.text2 }]} numberOfLines={1}>{m.label}</Text>
                  </View>
                </PressableScale>
              ))}
            </View>
...
const styles = StyleSheet.create({
  ...
  bannerTxt: { flex: 1, fontSize: 14, fontWeight: '800' },
  card: { borderRadius: 20, borderWidth: 1, padding: 16, marginBottom: 12 },
  season: { fontSize: 16, fontWeight: '800', marginBottom: 10 },
  dateRow: { flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 8 },
  dl: { fontSize: 12.5, fontWeight: '700', width: 52 },
  slash: { fontSize: 16, fontWeight: '800' },
  ...
  numVal: { fontSize: 18, fontWeight: '800', minWidth: 28, textAlign: 'center' },
