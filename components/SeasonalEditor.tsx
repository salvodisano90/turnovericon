        ...
        return (
          <View key={s.key} style={[styles.card, { backgroundColor: colors.card, borderColor: colors.line }]}>
            ...
            <View style={styles.actions}>
              ...
              {isCustom(sel) ? <PressableScale onPress={() => editCustom(sel as string)}><Text style={[styles.act, { color: colors.blue }]}>Modifica</Text></PressableScale> : null}
              {isCustom(sel) ? <PressableScale onPress={() => dupCustom(sel as string, s.key)}><Text style={[styles.act, { color: colors.blue }]}>Duplica</Text></PressableScale> : null}
            </View>
          </View>
        );
      })}
...
const styles = StyleSheet.create({
  ...
  hint: { fontSize: 12.5, fontWeight: '600', paddingVertical: 8 },
  actions: { flexDirection: 'row', flexWrap: 'wrap', gap: 16, marginTop: 14 },
  act: { fontSize: 13.5, fontWeight: '800' },
});
