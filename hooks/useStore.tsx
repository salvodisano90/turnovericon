...
import {
  AuditEntry, BuildStats, ProfileConfig, Cell, Coverage, Desiderata, EngineContext, Ferie, GenerationMode, PlanMode, Matrice, Piano, PianoStore, Reparto, Snapshot, Staff, Turno, UserRole, Membership, ApprovalRequest,
} from '../types';
...
import { can, isAuthorizedStaff, emailDuplicata } from '../utils/permissions';
import { recordAudit } from '../services/auditLog';
import { getOutbox } from '../services/syncBootstrap';
import { toSyncOp } from '../services/syncMapper';
import { Invitation, createInvitation, acceptInvitation, revokeInvitation, hasAcceptedInvite } from '../utils/invitations';
...
interface StoreContextValue {
  ...
  updateMember: (id: string, patch: Partial<Membership>) => void;
  removeMember: (id: string) => void;
  profile: ProfileConfig;
  setProfile: (patch: Partial<ProfileConfig>) => void;
  currentEmail?: string;
  loginAsStaff: (email: string) => { ok: boolean; error?: string };
  loginAsOwner: (email?: string) => void;
  inviteStaff: (email: string, ruolo: string) => Invitation;
  acceptStaffInvite: (email: string, token: string) => { ok: boolean; error?: string };
  revokeStaffInvite: (token: string) => void;
  logoutSession: () => void;
...
export function StoreProvider({ children }: { children: React.ReactNode }) {
      ...
      if (d) {
        dispatch({
          ...
          payload: {
            ...
            role: d.role || 'OWNER', members: d.members || [], requests: d.richieste || [], profile: d.profile || { kind: 'initials', color: '#79B8FF' }, currentEmail: d.currentEmail, invitations: d.invitations || [],
          },
        });
      } else {
  ...
  }, [state.reparti, state.staff, state.ferie, state.pianos, state.month, state.year, state.audit, state.history, state.future, state.desiderata, state.mode, state.aiMode, state.matriciCustom, state.matriceMese, state.role, state.members, state.requests, state.profile, state.currentEmail, state.hydrated]);
  ...
  const addReparto = useCallback((rep: Reparto) => {
    const s = stateRef.current;
    if (!can(s.role, 'editReparti')) return; // RBAC: solo Coordinatore
  ...
  const removeReparto = useCallback((id: string) => {
    ...
    recordAudit({ actor: s.currentEmail || 'owner', role: s.role || 'OWNER', azione: 'rimuovi_reparto', target: removed ? removed.nome : id });
  ...
  const addStaff = useCallback((member: Staff) => {
    const s = stateRef.current;
    if (!can(s.role, 'editPersonale')) return; // RBAC: solo Coordinatore
  ...
  const removeStaff = useCallback((id: string) => {
    ...
    recordAudit({ actor: s.currentEmail || 'owner', role: s.role || 'OWNER', azione: 'rimuovi_operatore', target: removed ? removed.nome : id });
  ...
  const setCell = useCallback(
    (infId: string, day: number, turno: Turno, repartoId: string | null, settore: string | null) => {
      const s = stateRef.current;
      if (!can(s.role, 'editTurni')) return; // RBAC: solo il Coordinatore (OWNER) modifica i turni
      const key = monthKey(s.year, s.month);
  ...
  const regenerate = useCallback((): { stats: BuildStats; coverage: Coverage } => {
    const s = stateRef.current;
    if (!can(s.role, 'editTurni')) return { stats: {} as BuildStats, coverage: {} as Coverage }; // RBAC: solo Coordinatore
    const c = ctxOf(s);
  ...
  const removeMember = useCallback((id: string) => { const s = stateRef.current; const m = s.members.find((x) => x.id === id); commit({ members: s.members.filter((x) => x.id !== id) }, 'delete', 'accesso', m ? m.email : id, null); }, [commit]);
  const updateMember = useCallback((id: string, patch: Partial<Membership>) => {
    const s = stateRef.current;
    const members = s.members.map((x) => (x.id === id ? { ...x, ...patch } : x));
    const m = members.find((x) => x.id === id);
    commit({ members }, 'update', 'accesso', id, m ? `${m.email} · ${m.stato}` : id);
  }, [commit]);

  // Login Staff locale: l'email deve essere fra gli utenti autorizzati e non disattivata. Nessun OTP (v1).
  const inviteStaff = useCallback((email: string, ruolo: string): Invitation => {
    const s = stateRef.current;
    const inv = createInvitation(email, ruolo, Date.now());
    merge({ invitations: [...(s.invitations || []), inv] });
    recordAudit({ actor: s.currentEmail || 'owner', role: s.role || 'OWNER', azione: 'invita_operatore', target: inv.email, dettagli: inv.token });
    return inv;
  }, [merge]);
  const acceptStaffInvite = useCallback((email: string, token: string): { ok: boolean; error?: string } => {
    const s = stateRef.current;
    const r = acceptInvitation(s.invitations || [], email, token, Date.now());
    if (r.ok) merge({ invitations: r.list });
    return { ok: r.ok, error: r.error };
  }, [merge]);
  const revokeStaffInvite = useCallback((token: string) => {
    const s = stateRef.current;
    merge({ invitations: revokeInvitation(s.invitations || [], token) });
  }, [merge]);

  const loginAsOwner = useCallback((email?: string) => {
    merge({ role: 'OWNER', currentEmail: email ? email.trim().toLowerCase() : undefined });
  }, [merge]);

  const loginAsStaff = useCallback((email: string): { ok: boolean; error?: string } => {
    const s = stateRef.current;
    const e = (email || '').trim().toLowerCase();
    if (!e) return { ok: false, error: 'Inserisci l\u2019email.' };
    if (emailDuplicata(s.members, e)) return { ok: false, error: 'Email associata a pi\u00f9 operatori: contatta il coordinatore.' };
    if (!isAuthorizedStaff(s.members, e) && !hasAcceptedInvite(s.invitations || [], e)) return { ok: false, error: 'Email non autorizzata dal coordinatore.' };
    const m = s.members.find((x) => (x.email || '').trim().toLowerCase() === e);
    merge({ role: 'STAFF', currentEmail: m ? m.email : e });
    return { ok: true };
  }, [merge]);

  const logoutSession = useCallback(() => { merge({ role: 'OWNER', currentEmail: undefined }); }, [merge]);
  ...
  const approveRequest = useCallback((id: string, commento?: string) => {
    const s = stateRef.current;
    if (!can(s.role, 'approve')) return; // RBAC: solo il Coordinatore approva
    const req = s.requests.find((r) => r.id === id); if (!req) return;
    recordAudit({ actor: s.currentEmail || 'owner', role: s.role || 'OWNER', azione: 'approva_richiesta', target: id, dettagli: commento });
  ...
  const rejectRequest = useCallback((id: string, commento?: string) => {
    const s = stateRef.current;
    if (!can(s.role, 'approve')) return; // RBAC: solo il Coordinatore respinge
    const req = s.requests.find((r) => r.id === id); if (!req) return;
    recordAudit({ actor: s.currentEmail || 'owner', role: s.role || 'OWNER', azione: 'rifiuta_richiesta', target: id, dettagli: commento });
