...
export function StoreProvider({ children }: { children: React.ReactNode }) {
      ...
      if (d) {
        dispatch({
          ...
          payload: {
            ...
            role: d.role || 'OWNER', members: d.members || [], requests: d.richieste || [], profile: d.profile || { kind: 'initials', color: '#79B8FF' }, currentEmail: d.currentEmail,
          },
        });
      } else {
  ...
  const loginAsStaff = useCallback((email: string): { ok: boolean; error?: string } => {
    ...
    if (!isAuthorizedStaff(s.members, e)) return { ok: false, error: 'Email non autorizzata dal coordinatore.' };
