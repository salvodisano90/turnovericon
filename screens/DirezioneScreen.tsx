...
export default function DirezioneScreen() {
  ...
  if (!ctx.staff.length || !ctx.reparti.length) {
    return (<View style={[styles.root, { backgroundColor: colors.bg }]}><SheetHeader title="Direzione" subtitle="Quadro aziendale" onClose={() => (router.canGoBack() ? router.back() : router.replace("/"))} /><EmptyState icon="stats-chart-outline" title="Dati insufficienti" desc="Configura reparti e personale per il quadro direzionale." /></View>);
  }
  ...
  const KPI = [
    ...
    { lab: 'Fedeltà matrice', val: mf ? `${mf.score}` : '—', icon: 'grid-outline', color: mf ? (mf.score >= 90 ? colors.green : mf.score >= 75 ? colors.blue : mf.score >= 60 ? colors.yellow : colors.red) : colors.text3 },
  ];

  return (
    <View style={[styles.root, { backgroundColor: colors.bg }]}>
      <SheetHeader title="Direzione" subtitle="Quadro aziendale del mese" onClose={() => (router.canGoBack() ? router.back() : router.replace("/"))} />
...
const styles = StyleSheet.create({
  ...
  violTxt: { fontSize: 13 },
  violN: { fontSize: 13, fontWeight: '800' },
});
