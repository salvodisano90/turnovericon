...
export default function FerieWizardScreen() {
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.card, paddingBottom: insets.bottom }]}>
      <SheetHeader title={isEdit ? 'Modifica Assenza' : 'Aggiungi Assenza'} subtitle={`${MONTHS[month]} ${year}`} onClose={() => (router.canGoBack() ? router.back() : router.replace("/"))} />
      ...
      <View style={[styles.foot, { borderTopColor: colors.separator }]}>
        <Button title="Annulla" variant="secondary" onPress={() => (router.canGoBack() ? router.back() : router.replace("/"))} style={{ flex: 1 }} />
...
const styles = StyleSheet.create({
  ...
  infoTxt: { fontSize: 12, fontWeight: '600', lineHeight: 18 },
