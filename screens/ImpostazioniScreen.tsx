...
export default function ImpostazioniScreen() {
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.bg, paddingBottom: insets.bottom }]}>
      <SheetHeader title="Impostazioni" subtitle="Modalità di generazione del motore" onClose={() => (router.canGoBack() ? router.back() : router.replace("/"))} />
      <ScrollView contentContainerStyle={styles.body}>
        <SectionTitle>Accessi</SectionTitle>
        <Button title="Utenti autorizzati" full icon="people-outline" variant="secondary" onPress={() => router.push('/utenti-autorizzati')} style={{ marginBottom: 6 }} />
...
const styles = StyleSheet.create({
  ...
  badge: { fontSize: 11, fontWeight: '800', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 10, overflow: 'hidden' },
  sub: { fontSize: 12, marginTop: 4, lineHeight: 18 },
  prio: { fontSize: 11, marginTop: 3 },
