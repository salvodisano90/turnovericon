...
export default function NotificheScreen() {
  ...
  const { requests, staff, approveRequest, rejectRequest } = useStore();
  const [rejectId, setRejectId] = useState<string | null>(null);
  const [comment, setComment] = useState('');

  const pending = useMemo(() => (requests || []).filter((r) => r.stato === 'pending').sort((a, b) => (b.createdAt || '').localeCompare(a.createdAt || '')), [requests]);
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.bg }]}>
      <SheetHeader title="Notifiche" subtitle={pending.length ? `${pending.length} da gestire` : 'Tutto gestito'} onClose={() => router.back()} />
        ...
        {pending.length ? pending.map((r) => (
          <GlassCard key={r.id} style={{ marginBottom: 10 }}>
            <View style={styles.row}>
              ...
              <View style={[styles.statePill, { backgroundColor: colors.yellowSoft }]}><Text style={[styles.stateTxt, { color: colors.yellow }]}>In attesa</Text></View>
            </View>
            {rejectId === r.id ? (
...
const styles = StyleSheet.create({
  ...
  input: { borderRadius: 10, paddingHorizontal: 12, paddingVertical: 10, fontSize: 14, borderWidth: 1, minHeight: 60, textAlignVertical: 'top' },
