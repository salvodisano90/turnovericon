...
export default function StaffWizardScreen() {
  ...
  const save = () => {
    ...
    const member: Staff = {
      ...
      anniEsperienza: anni > 0 ? anni : undefined,
      livello: livello || undefined,
      competenze: (() => {
        const prev = (editing && editing.competenze) || [];
        const merged: Competenza[] = comp.map((area) => prev.find((c) => c.area === area) || { area, livello: 'base' });
      ...
      <ScrollView contentContainerStyle={styles.body} keyboardShouldPersistTaps="handled">
        {step === 1 ? (
          <>
            ...
            <Text style={[styles.label, { color: colors.text, marginTop: 18 }]}>Anni di esperienza</Text>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
              <Stepper value={anni} onChange={(d) => setAnni(Math.min(Math.max(0, anni + d), 45))} />
              <Text style={{ color: colors.text2, fontSize: 13 }}>{anni} {anni === 1 ? 'anno' : 'anni'}</Text>
            </View>
            <Text style={[styles.label, { color: colors.text, marginTop: 16 }]}>Livello professionale</Text>
            <Text style={[styles.help, { color: colors.text3 }]}>Opzionale: se impostato, ha priorità sulla classificazione automatica. Non blocca la generazione.</Text>
            <View style={styles.chips}>
              <SelectChip label="Automatico" selected={!livello} onPress={() => setLivello('')} />
              {LIVELLI.map((l) => (
                <SelectChip key={l} label={l} selected={livello === l} onPress={() => setLivello(l)} />
              ))}
            </View>
            <Text style={[styles.label, { color: colors.text, marginTop: 16 }]}>Competenze (opzionali)</Text>
            <Text style={[styles.help, { color: colors.text3 }]}>Informative: non vincolano la generazione del piano.</Text>
            <View style={styles.chips}>
              {COMPETENZE_AREE.map((c) => (
                <SelectChip key={c} label={c} selected={comp.indexOf(c) >= 0} onPress={() => setComp((arr) => arr.indexOf(c) >= 0 ? arr.filter((x) => x !== c) : [...arr, c])} />
              ))}
            </View>
          </>
        ) : null}
              ...
              return (
                <OptionCard key={m.id} selected={matrice === m.id} onPress={() => setMatrice(m.id)}>
                  ...
                  <View style={styles.seq}>
                    {m.seq.map((tt, idx) => {
                      const sc = colors.shift[tt as Turno] || colors.shift.R;
                      return (
                      <View key={idx} style={[styles.seqBlk, { backgroundColor: sc.bg }]}>
                        <Text style={[styles.seqTxt, { color: sc.fg }]}>{tt}</Text>
                      </View>
                    ); })}
                  </View>
          ...
          <>
            ...
            <View style={styles.offPrev}>
              {Array.from({ length: 14 }).map((_, i) => {
                const d = i + 1;
                // FIX: il ciclo della matrice NON è sempre 7 giorni (Quinta=5, Sesta=6, Decima=10,
                // TM/TA/D12H=5, ...). Si usa la lunghezza reale della sequenza, con guardia anti-undefined.
                ...
                const sc = colors.shift[t] || colors.shift.R;
                return (
                  <View key={d} style={styles.offDay}>
                    <View style={[styles.offBlk, { backgroundColor: sc.bg }]}>
                      <Text style={[styles.offTxt, { color: sc.fg }]}>{t}</Text>
                    </View>
...
const styles = StyleSheet.create({
  ...
  help: { fontSize: 12, marginTop: -2, marginBottom: 8, lineHeight: 17 },
  ...
  optTitle: { fontSize: 13, fontWeight: '700' },
  optSub: { fontSize: 12, marginTop: 2 },
  optRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  optBadge: { fontSize: 11, fontWeight: '700', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 10, overflow: 'hidden' },
  ...
  sumK: { fontSize: 12 },
  sumV: { fontSize: 12, fontWeight: '600', flexShrink: 1, textAlign: 'right' },
