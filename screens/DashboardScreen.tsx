    ...
    <View style={[styles.root, { backgroundColor: colors.bg }]}>
      <ScrollView contentContainerStyle={{ paddingTop: insets.top + 8, paddingHorizontal: 16, paddingBottom: insets.bottom + 110 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.kpiRow}>
          ...
          <KPICard icon="people" label="Operatori" value={`${inService}/${ctx.staff.length}`} sub="In servizio" subColor={colors.green} />
        </View>
        <View style={styles.kpiRow}>
          <KPICard icon="alert-circle" label="Criticità" value={`${crit}`} sub="Aperte" subColor={crit ? colors.red : colors.green} onPress={() => router.push('/centro-criticita')} />
          <KPICard icon="scale-outline" label="Equità" value={fair ? `${fair.fairnessScore}` : '—'} sub={fair ? fair.categoria : 'In attesa di dati'} subColor={fairCol} />
        </View>
...
const styles = StyleSheet.create({
  hero: { borderRadius: 32, borderWidth: 1, padding: 24, minHeight: 260, marginBottom: 16 },
