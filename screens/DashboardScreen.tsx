...
export default function DashboardScreen() {
  ...
  const { ctx, currentPiano, profile } = useStore();
  const { user } = useAuth();
    ...
    <View style={[styles.root, { backgroundColor: colors.bg }]}>
      <ScrollView contentContainerStyle={{ paddingTop: insets.top + 8, paddingHorizontal: 16, paddingBottom: insets.bottom + 110 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.greetRow}>
          ...
          <View style={[styles.datePill, { backgroundColor: colors.card2 }]}><Text style={[styles.dateTxt, { color: colors.text2 }]}>{GIORNI[today.getDay()]} {dayNum} {MESI[today.getMonth()]}</Text></View>
        </View>
        ...
        <View style={styles.kpiRow}>
          <QuickActionCard icon="sparkles-outline" title="Genera Turni" subtitle="Piano mensile automatico" onPress={() => router.push('/')} />
          <QuickActionCard icon="people-outline" title="Personale" subtitle="Anagrafica operatori" onPress={() => router.push('/personale')} />
        </View>
        <View style={styles.kpiRow}>
          <QuickActionCard icon="alert-circle-outline" title="Centro Criticità" subtitle="Scoperture e problemi" badge={crit || undefined} onPress={() => router.push('/centro-criticita')} />
          <QuickActionCard icon="flask-outline" title="Assistente AI" subtitle="Analisi e suggerimenti" onPress={() => router.push('/assistente')} />
        </View>
      </ScrollView>
    </View>
  );
