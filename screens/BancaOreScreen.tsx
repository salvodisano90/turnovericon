...
export default function BancaOreScreen() {
  ...
  const { ctx, currentPiano, year, role, members, currentEmail } = useStore();

  const myInfId = useMemo(() => { const m = (members || []).find((x: any) => (x.email || '').trim().toLowerCase() === (currentEmail || '').trim().toLowerCase()); return m ? m.infId : undefined; }, [members, currentEmail]);
  const rows = useMemo(() => { const all = hoursBank(ctx, currentPiano || {}); return role === 'STAFF' ? all.filter((r) => r.infId === myInfId) : all; }, [ctx, currentPiano, role, myInfId]);
...
const styles = StyleSheet.create({
  ...
  saldo: { fontSize: 15, fontWeight: '800' },
