      ...
      <ScrollView contentContainerStyle={styles.body}>
        ...
        {step === 3 ? (
          <>
                ...
                {matriciCustom.map((m) => (
                  <OptionCard key={m.id} selected={matrice === m.id} onPress={() => setMatrice(m.id)}>
                    ...
                    <View style={styles.custActions}>
                      ...
                      ])}><Text style={[styles.custAct, { color: colors.text2 }]}>•••</Text></PressableScale>
                    </View>
                  </OptionCard>
                ))}
              </>
            ...
            {matMode === 'seasonal' ? (
              <SeasonalEditor cfg={seasonal} onChange={setSeasonal} />
            ) : null}
          </>
        ) : null}
...
const styles = StyleSheet.create({
  ...
  modeTxt: { fontSize: 13, fontWeight: '800' },
  custActions: { flexDirection: 'row', gap: 16, marginTop: 12 },
  custAct: { fontSize: 13, fontWeight: '800' },
  ...
  hint: { fontSize: 12, marginTop: 14, marginBottom: 8 },
  ...
  timeLabel: { fontSize: 22, fontWeight: '700', marginBottom: 6 },
  timeInput: { flex: 1, height: 44, borderWidth: 1, borderRadius: 10, paddingHorizontal: 12, fontSize: 15, textAlign: 'center' },
