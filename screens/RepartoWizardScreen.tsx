            ...
            {TURNS.map((t) => (
              <View key={t} style={[styles.timeBlock, { backgroundColor: colors.card, borderColor: colors.line }]}>
                ...
                <View style={styles.timeFields}>
                  <View style={{ flex: 1 }}><TimeStepperField label="Inizio" value={orari[t].s} onChange={(v) => setTime(t, 's', v)} /></View>
                  <View style={styles.vsep} />
                  <View style={{ flex: 1 }}><TimeStepperField label="Fine" value={orari[t].e} onChange={(v) => setTime(t, 'e', v)} /></View>
                </View>
              </View>
            ))}
...
const styles = StyleSheet.create({
  ...
  timeBlock: { borderWidth: 1, borderRadius: 24, padding: 14, marginBottom: 12 },
  vsep: { width: 1, alignSelf: 'stretch', backgroundColor: 'rgba(255,255,255,0.08)' },
  timeFields: { flexDirection: 'row', alignItems: 'stretch', gap: 12, marginTop: 6 },
  container: { flex: 1 },
  body: { padding: 16 },
