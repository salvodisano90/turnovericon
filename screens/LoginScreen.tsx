...
export default function LoginScreen() {
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.bg }]}>
      <SheetHeader title="Accedi" subtitle="Entra nel tuo reparto" onClose={() => (router.canGoBack() ? router.back() : router.replace("/"))} />
        ...
        <GlassCard>
          ...
          <View style={styles.segWrap}>
            {(([['manager', 'Manager'], ['staff', 'Staff']]) as Array<['manager' | 'staff', string]>).map(([mv, ml]) => (
              <PressableScale key={mv} onPress={() => { setMode(mv); setErr(null); }} style={{ flex: 1 }}>
                <View style={[styles.segBtn, { backgroundColor: mode === mv ? colors.card2 : 'transparent', borderColor: mode === mv ? colors.blue : colors.line }]}>
                  <Text style={[styles.segTxt, { color: mode === mv ? colors.blue : colors.text3 }]}>{ml}</Text>
                </View>
              </PressableScale>
            ))}
          </View>
          ...
          <TextInput style={inp(colors)} placeholder="••••••••" placeholderTextColor={colors.text3} secureTextEntry value={pwd} onChangeText={setPwd} editable={mode === 'manager'} />
...
const styles = StyleSheet.create({
  ...
  segBtn: { minHeight: 44, borderRadius: 14, borderWidth: 1, alignItems: 'center', justifyContent: 'center' },
